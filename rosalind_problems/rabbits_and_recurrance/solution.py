...
number of rabbits that were alive two months prior. As a result, if Fn represents the number of rabbit pairs alive
after the n-th month, then we obtain the Fibonacci sequence having terms Fn that are defined by the recurrence
...
def recurance_rabbits(months, init_pairs):
    actual_months = months - 2
    k_array = [1,1]
    for month in range(actual_months):
        f_new = k_array[-1] + (init_pairs * k_array[-2])
        k_array.append(f_new)
    return k_array[-1]
